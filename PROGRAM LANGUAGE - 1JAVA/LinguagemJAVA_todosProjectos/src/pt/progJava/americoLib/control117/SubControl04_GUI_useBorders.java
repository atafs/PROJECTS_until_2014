...
public class SubControl04_GUI_useBorders extends JFrame{
	...
	/*CONSTRUCTOR*/
	public SubControl04_GUI_useBorders() {
		...
		for (int i = 0; i < SIZE_OF_WINDOWS; i++) {
			square[i] = new JPanel();
			square[i].addMouseListener(new SubControl04_MouseListener(i, square[i]));
			
			//TEST -----------------------------------------
			ImageIcon icon = new ImageIcon(getClass().getResource("images/wall1.jpg"));
			JLabel jLabel = new JLabel();
			jLabel.setIcon(icon);
			jLabel.getMaximumSize();
			jLabel.setHorizontalAlignment(square[i].getX());
			jLabel.setVerticalAlignment(square[i].getY());		
			square[i].add(jLabel);
			//-----------------------------------------------
		...
		add(mainWindows);
		addMouseListener(new SubControl04_MouseListener(mainWindows));
